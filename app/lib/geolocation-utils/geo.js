...
export function insidePolygon (location, polygon) {
  ...
  return [toLonLatTuple(location), polygon.map(toLonLatTuple)]
}
...
export function getBoundingBox (locations, margin = 0) {
  ...
  // Get max / min values in array
  // lat  = top/bottom
  // long = left/right
  const top    = Math.max(...locations.map(getLatitude))
  const bottom = Math.min(...locations.map(getLatitude))
  const right  = Math.max(...locations.map(getLongitude))
  const left   = Math.min(...locations.map(getLongitude))
  ...
  const topLeft = createLocation(top, left, type)
  const topRight = createLocation(top, right, type)
  const bottomLeft = createLocation(bottom, left, type)
  const bottomRight = createLocation(bottom, right, type)
  ...
  if (margin === null || margin === 0) {
    ...
    return { 
      ...
      topRight: moveTo(topRight, {heading: 45, distance}),
      bottomLeft: moveTo(bottomLeft, {heading: 225, distance}),
